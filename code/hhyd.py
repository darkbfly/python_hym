...
cron: 30 */15 8-22 * * *
new Env('f花花阅读');
活动入口微信打开：http://mr1694734560441.kdqtcky.cn/user/index.html?mid=QS5PQAEZH
下载地址：https://www.123pan.com/s/xzeSVv-IHpfv.html
公告地址：http://175.24.153.42:8881/getmsg?type=ybxkhh

使用方法：
1.活动入口,微信打开：http://mr1694734560441.kdqtcky.cn/user/index.html?mid=QS5PQAEZH
2.打开活动链接，抓包接口http://u.cocozx.cn/api/user/info接口的请求体中的un和token参数
...
参数说明与获取：
ck:打开活动链接，抓包接口http://u.cocozx.cn/api/user/info接口的请求体中的un和token参数
...
class WXYD:
    ...
    def setstatus(self):
        try:
            ...
            p = {"key": self.key, "type": "ybxkhh", "val": "1"}
    ...
    def info(self):
        ...
        if rj.get("code") == 0:
            ...
            if hopeNull:
                ...
                print(self.name, f"当前账号今日已经阅读{dayCount}篇文章，剩余花花{self.moneyCurrent}")
                return False
            print(self.name, f"当前账号今日已经阅读{dayCount}篇文章，剩余花花{self.moneyCurrent}")
            self.statAccess()
            print(self.name, "-" * 50)
    ...
    def psmoneyc(self):
        ...
        rj = r.json()
        try:
            print("收取徒弟奖励:", rj.get("result").get("val"))
        except:
            print(rj)
    ...
    def testCheck(self, a, url):
        if checkDict.get(a[4]) != None:
            ...
            for i in range(60):
                if i % 30 == 0:
                    push(
                        f"{self.name}:花花阅读过检测", url, a[3], "ybxkhh", self.uids, self.key
                    )
    ...
    def submit(self):
        ...
        if rj.get("code") == 0:
            ...
            print(self.name, f'获得{result.get("val")}花花')
    ...
    def wdmoney(self):
        if self.moneyCurrent < txbz:
            print(self.name, "你的花花剩余不多了，不能提现")
            return False
            ...
            txm = 50000
        u = "http://u.cocozx.cn/api/user/wd"
...
if __name__ == "__main__":
    ...
    try:
        ...
        exit(0)
    zhconfig = os.getenv("hh_config")
    if zhconfig == None:
        print("请检查你的花花阅读脚本变量名称是否填写正确")
        exit(0)
    try:
