...
cron: 30 */15 8-22 * * *
new Env('f星空阅读');
活动入口微信打开：http://mr1692169663544.uznmvev.cn/ox/index.html?mid=XG7QUVEEH
下载地址：https://www.123pan.com/s/xzeSVv-IHpfv.html
公告地址：http://175.24.153.42:8881/getmsg?type=xingkong
星空阅读阅读：http://mr1692169663544.uznmvev.cn/ox/index.html?mid=XG7QUVEEH

使用方法：
1.活动入口,微信打开：http://mr1692169663544.uznmvev.cn/ox/index.html?mid=XG7QUVEEH
2.打开活动链接，抓包接口http://u.cocozx.cn/api/ox/info接口的请求体中的un和token参数
...
参数说明与获取：
ck:打开活动链接，抓包接口http://u.cocozx.cn/api/ox/info接口的请求体中的un和token参数
...
class WXYD:
    ...
    def info(self):
        ...
        if rj.get("code") == 0:
            ...
            if hopeNull:
                ...
                print(self.name, f"当前账号今日已经阅读{dayCount}篇文章，剩余元宝{self.moneyCurrent}")
                return False
            print(self.name, f"当前账号今日已经阅读{dayCount}篇文章，剩余元宝{self.moneyCurrent}")
            self.statAccess()
            print(self.name, "-" * 50)
    ...
    def wdmoney(self):
        if self.moneyCurrent < 10000:
            print(self.name, "你的元宝剩余不多了，不能提现")
            return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        exit(0)
    zhconfig = os.getenv("xk_config")
    if zhconfig == None:
        print("请检查你的星空阅读脚本变量名称是否填写正确")
        exit(0)
    try:
